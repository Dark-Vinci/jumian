...
class Application {
  ...
  private static readonly options = {
    url: RPCUrl.SALES,
    package: RPCServiceName.SALES,

    protoPath: join(__dirname, ProtoPath.SALES),
    loader: {
      enums: String,
      objects: true,
      arrays: true,
      ...
      default: {
        ...
        const app = await NestFactory.createMicroservice(AppModule, {
          ...
          logger: WinstonModule.createLogger({
            instance: initWinston(RPCServiceName.SALES),
          }),
        });

        await app.listen();
        Logger.log(
          `Application running on url localhost:${3004}, on worker with pid: ${
            process.pid
          }`,
        );
      }
